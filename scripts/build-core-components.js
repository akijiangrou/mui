...
function build(cb) {
  ...
  glob('**/*.*', { cwd: path.resolve(__dirname, '../src/core/components') }, (err, files) => {
    const filesToProcess = files.filter((file) => {
      ...
      if (file === 'xui.less') return false;
      if (file === 'xui.d.ts') return false;
    ...
    filesToProcess.forEach((file, index) => {
      ...
      if (file.indexOf('app.less') >= 0) {
        const iconsFontBase64 = base64Encode('./src/core/icons/font/xui-core-icons.woff');
        fileContent = fileContent.replace('xui_coreIconsFont()', `'${iconsFontBase64}'`);
      }
