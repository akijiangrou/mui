...
async function modular({ components }) {
  ...
  const coreSrc = fs.readFileSync(path.resolve(__dirname, '../src/core/xui.js'), 'utf-8');
  const liteSrc = fs.readFileSync(path.resolve(__dirname, '../src/core/xui-lite.js'), 'utf-8');
  ...
  // Save core
  fs.writeFileSync(`${outputDir}/xui.esm.js`, coreContent);
  // Save bundle
  fs.writeFileSync(`${outputDir}/xui-bundle.esm.js`, bundleContent);

  // Save lite
  fs.writeFileSync(`${outputDir}/xui-lite.esm.js`, liteContent);
  // Save lite bundle
  fs.writeFileSync(`${outputDir}/xui-lite-bundle.esm.js`, liteBundleContent);

  const files = ['xui.esm.js', 'xui-bundle.esm.js', 'xui-lite.esm.js', 'xui-lite-bundle.esm.js'];
...
async function umdBundle({ components } = {}) {
  ...
  return rollup({
    ...
    .then((bundle) => {
      ...
      return bundle.write({
        strict: true,
        file: `${output}/xui-bundle.js`,
        format: 'umd',
        name: 'Xui',
        sourcemap: env === 'production',
        sourcemapFile: `${output}/xui-bundle.js.map`,
        banner,
      });
    })
    .then(async (bundle) => {
      ...
      const minified = await minify(result.code, {
        sourceMap: {
          ...
          filename: env === 'production' ? 'xui-bundle.min.js' : undefined,
          url: `xui-bundle.min.js.map`,
        },
        output: {
          preamble: banner,
        },
      ...
      fs.writeFileSync(`${output}/xui-bundle.min.js`, minified.code);
      fs.writeFileSync(`${output}/xui-bundle.min.js.map`, minified.map);
    })
    .catch((err) => {
      console.log(err);
    });
  ...
  return rollup({
    ...
    .then((bundle) => {
      ...
      return bundle.write({
        ...
        name: 'Xui',
        sourcemap: env === 'production',
        sourcemapFile: `${output}/xui.js.map`,
        banner,
      });
    })
    .then(async (bundle) => {
      ...
      const minified = await minify(result.code, {
        sourceMap: {
          content: env === 'production' ? result.map : undefined,
          filename: env === 'production' ? 'xui.min.js' : undefined,
          url: `xui.min.js.map`,
        },
        output: {
          preamble: banner,
      ...
      fs.writeFileSync(`${output}/xui.min.js`, minified.code);
      fs.writeFileSync(`${output}/xui.min.js.map`, minified.map);
    })
    .catch((err) => {
      console.log(err);
    });
