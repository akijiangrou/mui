...
function copyLess(config, components, cb) {
  ...
  // Core LESS
  let lessContent = fs.readFileSync(path.resolve(__dirname, '../src/core/xui.less'));
  lessContent = `${banner}\n${lessContent}`;
  ...
  fs.writeFileSync(`${output}/xui.less`, lessContent);
  ...
  fs.writeFileSync(`${output}/xui-bundle.less`, lessBundleContent);
...
async function buildBundle(config, components, themes, rtl, cb) {
  ...
  const includeDarkTheme = config.darkTheme;
  const includeLightTheme = config.lightTheme;
  const outputFileName = `xui-bundle${rtl ? '-rtl' : ''}`;
  const output = `${getOutput()}/core`;
...
async function buildCore(themes, rtl, cb) {
  ...
  // Write file
  fs.writeFileSync(`${output}/xui${rtl ? '-rtl' : ''}.css`, `${banner}\n${cssContent}`);
  ...
  // Write file
  fs.writeFileSync(`${output}/xui${rtl ? '-rtl' : ''}.min.css`, `${banner}\n${minifiedContent}`);
