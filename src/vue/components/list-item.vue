...
const ListItemContent = ({
  ...
  if (after || badge || slots.after) {
    ...
    if (badge) {
      badgeEl = h(cbxBadge, { color: badgeColor }, () => badge);
    }
...
export default {
  ...
  setup(props, { slots, emit }) {
    ...
    watch(
      ...
      (newValue) => {
        if (!props.swipeout || !elRef.value || !cbx) return;
        if (newValue) {
          cbx.swipeout.open(elRef.value);
        } else {
          cbx.swipeout.close(elRef.value);
        }
      },
    );
    ...
    onMounted(() => {
      cbxready(() => {
        if (props.swipeout) {
          cbx.on('swipeoutOpen', onSwipeoutOpen);
          cbx.on('swipeoutOpened', onSwipeoutOpened);
          cbx.on('swipeoutClose', onSwipeoutClose);
          cbx.on('swipeoutClosed', onSwipeoutClosed);
          cbx.on('swipeoutDelete', onSwipeoutDelete);
          cbx.on('swipeoutDeleted', onSwipeoutDeleted);
          cbx.on('swipeoutOverswipeEnter', onSwipeoutOverswipeEnter);
          cbx.on('swipeoutOverswipeExit', onSwipeoutOverswipeExit);
          cbx.on('swipeout', onSwipeout);
        }
        if (props.accordionItem) {
          cbx.on('accordionBeforeOpen', onAccBeforeOpen);
          cbx.on('accordionOpen', onAccOpen);
          cbx.on('accordionOpened', onAccOpened);
          cbx.on('accordionBeforeClose', onAccBeforeClose);
          cbx.on('accordionClose', onAccClose);
          cbx.on('accordionClosed', onAccClosed);
        }
    ...
    onBeforeUnmount(() => {
      if (!cbx) return;
      cbx.off('swipeoutOpen', onSwipeoutOpen);
      cbx.off('swipeoutOpened', onSwipeoutOpened);
      cbx.off('swipeoutClose', onSwipeoutClose);
      cbx.off('swipeoutClosed', onSwipeoutClosed);
      cbx.off('swipeoutDelete', onSwipeoutDelete);
      cbx.off('swipeoutDeleted', onSwipeoutDeleted);
      cbx.off('swipeoutOverswipeEnter', onSwipeoutOverswipeEnter);
      cbx.off('swipeoutOverswipeExit', onSwipeoutOverswipeExit);
      cbx.off('swipeout', onSwipeout);
      cbx.off('accordionBeforeOpen', onAccBeforeOpen);
      cbx.off('accordionOpen', onAccOpen);
      cbx.off('accordionOpened', onAccOpened);
      cbx.off('accordionBeforeClose', onAccBeforeClose);
      cbx.off('accordionClose', onAccClose);
      cbx.off('accordionClosed', onAccClosed);
    });
