  ...
  setup(props, { emit }) {
    ...
    const setTooltip = () => {
      ...
      if (!hasVisibleDataSets) {
        if (cbxTooltip && cbxTooltip.hide) cbxTooltip.hide();
        return;
      }
      ...
      if (index !== null && !cbxTooltip) {
        ...
        if (cbxTooltip && cbxTooltip.show) {
          cbxTooltip.show();
        }
        return;
      }
      if (!cbxTooltip || !cbxTooltip.hide || !cbxTooltip.show) {
        return;
      }
      if (index !== null) {
        cbxTooltip.setText(formatTooltip());
        cbxTooltip.setTargetEl(svgElRef.value.querySelector(`line[data-index="${index}"]`));
        cbxTooltip.show();
      } else {
        cbxTooltip.hide();
      }
