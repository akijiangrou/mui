...
export default {
  ...
  setup(props, { emit }) {
    ...
    watch(
      () => props.opened,
      (value) => {
        if (!cbxPopup.value) return;
        if (value) {
          cbxPopup.value.open();
        } else {
          cbxPopup.value.close();
        }
      },
    );
    ...
    onMounted(() => {
      ...
      cbxready(() => {
        cbxPopup.value = cbx.popup.create(popupParams);
        if (props.opened) {
          cbxPopup.value.open(false);
        }
      });
    });
