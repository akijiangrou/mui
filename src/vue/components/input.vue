...
import cbxToggle from './toggle.js';
import cbxRange from './range.js';
import cbxTextEditor from './text-editor.js';
...
export default {
  ...
  setup(props, { emit, slots }) {
    let cbxCalendar = null;
    let cbxColorPicker = null;
    ...
    const validateInput = () => {
      if (!cbx || !inputElRef.value) return;
    ...
    onMounted(() => {
      ...
      cbxready(() => {
        ...
        if (type === 'datepicker') {
          cbxCalendar = cbx.calendar.create({
        ...
        if (type === 'colorpicker') {
          cbxColorPicker = cbx.colorPicker.create({
            inputEl: inputElRef.value,
            value,
            on: {
        ...
        cbx.input.checkEmptyState(inputElRef.value);
        ...
        if (resizable) {
          cbx.input.resizeTextarea(inputElRef.value);
        }
      });
    });
    ...
    onBeforeUnmount(() => {
      ...
      if (cbxCalendar && cbxCalendar.destroy) {
        cbxCalendar.destroy();
        cbxCalendar = null;
      }
      if (cbxColorPicker && cbxColorPicker.destroy) {
        cbxColorPicker.destroy();
        cbxColorPicker = null;
      }
    });
    ...
    watch(
      ...
      (newValue) => {
        if (!cbx || !cbxColorPicker) return;
        extend(cbxColorPicker.params, newValue || {});
      },
    );
    ...
    watch(
      ...
      (newValue) => {
        if (!cbx || !cbxCalendar) return;
        extend(cbxCalendar.params, newValue || {});
      },
    );
    ...
    watch(
      () => props.value,
      (newValue) => {
        if (props.type === 'range' || props.type === 'toggle' || !cbx) return;
        updateInputOnDidUpdate = true;
        if (cbxCalendar) {
          cbxCalendar.setValue(newValue);
        }
        if (cbxColorPicker) {
          cbxColorPicker.setValue(newValue);
        }
      },
    );
