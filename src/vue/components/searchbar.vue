...
export default {
  ...
  setup(props, { emit }) {
    let cbxSearchbar = null;
    const elRef = ref(null);

    const search = (query) => {
      if (!cbxSearchbar) return undefined;
      return cbxSearchbar.search(query);
    };
    const enable = () => {
      if (!cbxSearchbar) return undefined;
      return cbxSearchbar.enable();
    };
    const disable = () => {
      if (!cbxSearchbar) return undefined;
      return cbxSearchbar.disable();
    };
    const toggle = () => {
      if (!cbxSearchbar) return undefined;
      return cbxSearchbar.toggle();
    };
    const clear = () => {
      if (!cbxSearchbar) return undefined;
      return cbxSearchbar.clear();
    };
    ...
    onMounted(() => {
      ...
      cbxready(() => {
        ...
        cbxSearchbar = cbx.searchbar.create(params);
      });
    });

    onBeforeUnmount(() => {
      if (cbxSearchbar && cbxSearchbar.destroy) cbxSearchbar.destroy();
      cbxSearchbar = null;
    });
