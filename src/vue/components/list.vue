...
export default {
  ...
  setup(props, { emit, slots }) {
    ...
    onMounted(() => {
      cbxready(() => {
        cbx.on('sortableEnable', onSortableEnable);
        cbx.on('sortableDisable', onSortableDisable);
        cbx.on('sortableSort', onSortableSort);
        cbx.on('sortableMove', onSortableMove);
        ...
        cbxVirtualList = cbx.virtualList.create(
          extend(
            {
              el: elRef.value,
              on: {
    ...
    onBeforeUnmount(() => {
      if (!cbx) return;
      cbx.off('sortableEnable', onSortableEnable);
      cbx.off('sortableDisable', onSortableDisable);
      cbx.off('sortableSort', onSortableSort);
      cbx.off('sortableMove', onSortableMove);

      if (!(props.virtualList && cbxVirtualList)) return;
      if (cbxVirtualList.destroy) cbxVirtualList.destroy();
      cbxVirtualList = null;
    });
        ...
        slotsDefault().forEach((vnode) => {
          ...
          if (tag && typeof tag === 'symbol') {
            ...
            (tag &&
              !(
                tag === 'li' ||
                tag.indexOf('cbx-list-item') >= 0 ||
                tag.indexOf('cbx-list-button') >= 0 ||
                tag.indexOf('cbx-list-input') >= 0
              ))
          ) {
