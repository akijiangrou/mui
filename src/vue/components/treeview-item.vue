...
export default {
  ...
  setup(props, { slots, emit }) {
    ...
    const attachEvents = () => {
      ...
      cbxready(() => {
        cbx.on('treeviewOpen', onOpen);
        cbx.on('treeviewClose', onClose);
        cbx.on('treeviewLoadChildren', onLoadChildren);
      });
    };

    const detachEvents = () => {
      if (!cbx) return;
      cbx.off('treeviewOpen', onOpen);
      cbx.off('treeviewClose', onClose);
      cbx.off('treeviewLoadChildren', onLoadChildren);
    };
