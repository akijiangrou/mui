...
class Autocomplete extends XuiClass {
  constructor(app, params = {}) {
    ...
    if (ac.params.openerEl) {
      ...
      if ($openerEl.length) $openerEl[0].cbxAutocomplete = ac;
    }

    let $inputEl;
    if (ac.params.inputEl) {
      $inputEl = $(ac.params.inputEl);
      if ($inputEl.length) $inputEl[0].cbxAutocomplete = ac;
    }
  ...
  get view() {
    ...
    if (ac.params.view) {
      ...
      view = $el.closest('.view').length && $el.closest('.view')[0].cbxView;
    }
  ...
  destroy() {
    ...
    if (ac.$inputEl && ac.$inputEl[0]) {
      delete ac.$inputEl[0].cbxAutocomplete;
    }
    if (ac.$openerEl && ac.$openerEl[0]) {
      delete ac.$openerEl[0].cbxAutocomplete;
    }
    deleteProps(ac);
    ac.destroyed = true;
  }
