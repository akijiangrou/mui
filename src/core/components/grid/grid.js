    ...
    function handleTouchMove(e) {
      ...
      if (!isMoved) {
        ...
        if ($prevResizableEl.length) {
          ...
          gapSize = parseFloat(
            $prevResizableEl.css(isRow ? '--cbx-grid-row-gap' : '--cbx-grid-gap'),
          );
        }
        if ($nextResizableEl.length) {
          ...
          if (!$prevResizableEl.length) {
            ...
            itemsInFlow = $nextResizableEl
              ...
              .children(isRow ? '.row' : '[class*="col-"], .col').length;
            gapSize = parseFloat(
              $nextResizableEl.css(isRow ? '--cbx-grid-row-gap' : '--cbx-grid-gap'),
            );
          }
        }
      }
      ...
      const gapAddSizeCSS = isRow
        ? `${itemsInFlow - 1} * var(--cbx-grid-row-gap) / ${itemsInFlow}`
        : '(var(--cbx-cols-per-row) - 1) * var(--cbx-grid-gap) / var(--cbx-cols-per-row)';
