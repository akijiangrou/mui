...
const InfiniteScroll = {
  handle(el, e) {
    ...
    if (onTop) {
      ...
      if (virtualListContainer.length > 0) {
        virtualList = virtualListContainer.eq(-1)[0].cbxVirtualList;
  ...
  create(el) {
    ...
    $el.each((element) => {
      element.cbxInfiniteScrollHandler = scrollHandler;
      element.addEventListener('scroll', element.cbxInfiniteScrollHandler);
    });
  },
  destroy(el) {
    const $el = $(el);
    $el.each((element) => {
      element.removeEventListener('scroll', element.cbxInfiniteScrollHandler);
      delete element.cbxInfiniteScrollHandler;
    });
  },
};
export default {
