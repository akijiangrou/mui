...
class AreaChart extends XuiClass {
  constructor(app, params = {}) {
    ...
    if ($el[0].cbxAreaChart) return $el[0].cbxAreaChart;
  ...
  setTooltip() {
    ...
    if (!hasVisibleDataSets) {
      if (self.cbxTooltip && self.cbxTooltip.hide) self.cbxTooltip.hide();
      return;
    }
    ...
    if (currentIndex !== null && !self.cbxTooltip) {
      ...
      if (self.cbxTooltip && self.cbxTooltip.show) {
        self.cbxTooltip.show();
      }
      return;
    }
    if (!self.cbxTooltip || !self.cbxTooltip.hide || !self.cbxTooltip.show) {
      return;
    }
    if (currentIndex !== null) {
      self.cbxTooltip.setText(self.formatTooltip());
      self.cbxTooltip.setTargetEl(svgEl.querySelector(`line[data-index="${currentIndex}"]`));
      self.cbxTooltip.show();
    } else {
