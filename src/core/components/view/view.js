...
function getCurrentView(app) {
  ...
  if ($popoverView.length > 0 && $popoverView[0].cbxView) return $popoverView[0].cbxView;
  if ($popupView.length > 0 && $popupView[0].cbxView) return $popupView[0].cbxView;
  if ($panelView.length > 0 && $panelView[0].cbxView) return $panelView[0].cbxView;
  if ($viewEl.length > 0) {
    if ($viewEl.length === 1 && $viewEl[0].cbxView) return $viewEl[0].cbxView;
    if ($viewEl.length > 1) {
  ...
  create() {
    ...
    extend(app, {
      views: extend([], {
        ...
        get(viewEl) {
          const $viewEl = $(viewEl);
          if ($viewEl.length && $viewEl[0].cbxView) return $viewEl[0].cbxView;
          return undefined;
        },
      }),
    });
