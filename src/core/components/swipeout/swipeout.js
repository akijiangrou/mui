  ...
  init() {
    ...
    function handleTouchMove(e) {
      ...
      if ($actionsRight.length > 0) {
        ...
        $rightButtons.each((buttonEl) => {
          ...
          if (typeof buttonEl.cbxSwipeoutButtonOffset === 'undefined') {
            $buttonEl[0].cbxSwipeoutButtonOffset = buttonEl.offsetLeft;
          }
          buttonOffset = buttonEl.cbxSwipeoutButtonOffset;
          if (
      ...
      if ($actionsLeft.length > 0) {
        ...
        $leftButtons.each((buttonEl, index) => {
          const $buttonEl = $(buttonEl);
          if (typeof buttonEl.cbxSwipeoutButtonOffset === 'undefined') {
            $buttonEl[0].cbxSwipeoutButtonOffset =
    ...
    function handleTouchEnd() {
      ...
      if (action === 'open') {
        ...
        if (overswipeRight) {
          $actionsRight.find('.swipeout-overswipe').trigger('click', 'cbxOverswipe');
        }
        if (overswipeLeft) {
          $actionsLeft.find('.swipeout-overswipe').trigger('click', 'cbxOverswipe');
        }
      } else {
  ...
  delete(el, callback) {
    ...
    $el.transitionEnd(() => {
      ...
      if ($el.parents('.virtual-list').length > 0) {
        const virtualList = $el.parents('.virtual-list')[0].cbxVirtualList;
        const virtualIndex = $el[0].cbxVirtualListIndex;
