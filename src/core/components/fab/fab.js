...
const Fab = {
  morphOpen(fabEl, targetEl) {
    ...
    $fabEl[0].cbxFabMorphResizeHandler = function resizeHandler() {
    ...
    $fabEl.transitionEnd(() => {
      ...
      app.on('resize', $fabEl[0].cbxFabMorphResizeHandler);
      if ($targetEl.parents('.page-content').length > 0) {
        $targetEl.parents('.page-content').on('scroll', $fabEl[0].cbxFabMorphResizeHandler);
      }
    });
  },
  morphClose(fabEl) {
    const app = this;
    const $fabEl = $(fabEl);
    const morphData = $fabEl[0].cbxFabMorphData;
    if (!morphData) return;
    ...
    app.off('resize', $fabEl[0].cbxFabMorphResizeHandler);
    if ($targetEl.parents('.page-content').length > 0) {
      $targetEl.parents('.page-content').off('scroll', $fabEl[0].cbxFabMorphResizeHandler);
    }
