...
const Lazy = {
  destroy(pageEl) {
    ...
    if ($pageEl[0].cbxLazyDestroy) {
      $pageEl[0].cbxLazyDestroy();
    }
  },
  create(pageEl) {
    const app = this;
    ...
    if (app.params.lazy.observer && support.intersectionObserver) {
      let observer = $pageEl[0].cbxLazyObserver;
      if (!observer) {
      ...
      $lazyLoadImages.each((el) => {
        if (el.cbxLazyObserverAdded) return;
        el.cbxLazyObserverAdded = true;
        observer.observe(el);
      });
      if (!$pageEl[0].cbxLazyDestroy) {
        $pageEl[0].cbxLazyDestroy = () => {
          observer.disconnect();
          delete $pageEl[0].cbxLazyDestroy;
          delete $pageEl[0].cbxLazyObserver;
        };
      }
      return;
    }
    ...
    function detachEvents() {
      $pageEl[0].cbxLazyAttached = false;
      delete $pageEl[0].cbxLazyAttached;
    ...
    if (!$pageEl[0].cbxLazyDestroy) {
      $pageEl[0].cbxLazyDestroy = detachEvents;
    }

    // Attach events
    if (!$pageEl[0].cbxLazyAttached) {
      attachEvents();
    }
