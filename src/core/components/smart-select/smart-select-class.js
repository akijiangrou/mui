...
class SmartSelect extends XuiClass {
  constructor(app, params = {}) {
    ...
    if ($el[0].cbxSmartSelect) return $el[0].cbxSmartSelect;
  ...
  get view() {
    ...
    if (!view) {
      view = $el.parents('.view').length && $el.parents('.view')[0].cbxView;
    }
  ...
  renderItem(item, index) {
    ...
    function getIconClass(iconValue = '') {
      if (iconValue.indexOf(':') >= 0) {
        ...
        if (className === 'cbx') className = 'cbx-icons';
