...
const textEditorButtonsMap = {
  // cbx-icon, material-icon, command
...
class TextEditor extends XuiClass {
  constructor(app, params) {
    ...
    if ($el[0].cbxTextEditor) return $el[0].cbxTextEditor;
    ...
    if (typeof self.params.buttons === 'string') {
      try {
        ...
        throw new Error('Xui: TextEditor: wrong "buttons" parameter format');
      }
    }

    $el[0].cbxTextEditor = self;
  ...
  renderButtons() {
    ...
    function renderButton(button) {
      const iconClass = self.app.theme === 'md' ? 'material-icons' : 'cbx-icons';
  ...
  destroy() {
    ...
    delete self.$el[0].cbxTextEditor;
    deleteProps(self);
    self = null;
  }
}
