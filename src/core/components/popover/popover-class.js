...
class Popover extends Modal {
  constructor(app, params) {
    ...
    if (popover.params.backdrop && popover.params.backdropEl) {
      ...
      if (popover.params.backdropUnique || forceBackdropUnique) {
        ...
        $backdropEl[0].cbxPopoverRef = popover;
      ...
      if ($target.closest(popover.el).length === 0) {
        if (
          ...
          const isAnotherPopoverBackdrop =
            ($target.hasClass('popover-backdrop-unique') && target.cbxPopoverRef !== popover) ||
  ...
  resize() {
    ...
    let targetOffsetTop;
    let safeAreaTop = parseInt($('html').css('--cbx-safe-area-top'), 10);
    let safeAreaLeft = parseInt($('html').css('--cbx-safe-area-left'), 10);
    let safeAreaRight = parseInt($('html').css('--cbx-safe-area-right'), 10);
