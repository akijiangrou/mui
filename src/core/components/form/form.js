...
const FormData = {
  store(form, data) {
    ...
    window.localStorage[`cbxform-${formId}`] = JSON.stringify(data);
  },
  get(form) {
    const app = this;
    const window = getWindow();
    ...
    if (window.localStorage[`cbxform-${formId}`]) {
      return JSON.parse(window.localStorage[`cbxform-${formId}`]);
    }
  ...
  remove(form) {
    ...
    if (window.localStorage[`cbxform-${formId}`]) {
      window.localStorage[`cbxform-${formId}`] = '';
      window.localStorage.removeItem(`cbxform-${formId}`);
    }
  },
};
