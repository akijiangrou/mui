...
export interface XuiEventsClass<Events> {
  /** Add event handler */
  on<E extends keyof Events>(event: E, handler: Events[E]): void;
  /** Add event handler that will be removed after it was fired */
  once<E extends keyof Events>(event: E, handler: Events[E]): void;
...
export interface XuiParameters {
  ...
  el?: string;
  /** App bundle id.. (default io.xui.testapp) */
  id?: string | 'io.xui.testapp';
  /** App name. Can be used by other components, e.g. as the default title for Dialog component.. (default Xui) */
  name?: string;
  ...
  /** By default Xui will be initialized automatically when you call new Xui(). If you want to prevent this behavior you can disable it with this option and then initialize it manually with init() when you need it.. (default true) */
  init?: boolean;
  ...
  on?: {
    [event in keyof XuiEvents]?: XuiEvents[event];
  };
}

export interface XuiPlugin {
  /** Module Name */
  name: string;
  /** Install callback
...
export interface XuiEvents {
  /** Event will be fired on app initialization. Automatically after new Xui() or after app.init() if you disabled auto init. */
  init: () => void;
...
interface Xui extends XuiClass<XuiEvents> {
  ...
  init(): void;
  /** Load module */
  loadModule(module: string | Function | XuiPlugin): Promise<any>;
  /** Load modules */
  loadModules(modules: any[]): Promise<any>;
}
interface Events extends XuiEventsClass<XuiEvents> {}

declare class Xui implements Xui {
  constructor(parameters?: XuiParameters);

  static use(plugin: XuiPlugin): void;
  static use(plugins: XuiPlugin[]): void;
