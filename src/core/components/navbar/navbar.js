...
const Navbar = {
  size(el) {
    ...
    if ($viewEl.length > 0 && $viewEl[0].cbxView) {
      router = $viewEl[0].cbxView.router;
    ...
    if (dynamicNavbar && app.theme === 'ios') {
      if (title.hasClass('sliding') || (title.length > 0 && sliding)) {
        ...
        title[0].cbxNavbarLeftOffset = titleLeftOffset;
        title[0].cbxNavbarRightOffset = titleRightOffset;
      }
      if (!noLeft && (left.hasClass('sliding') || sliding)) {
        if (app.rtl) {
          left[0].cbxNavbarLeftOffset = (-(navbarInnerWidth - left[0].offsetWidth) / 2) * inverter;
          left[0].cbxNavbarRightOffset = leftWidth * inverter;
        } else {
          left[0].cbxNavbarLeftOffset = -leftWidth;
          left[0].cbxNavbarRightOffset = (navbarInnerWidth - left[0].offsetWidth) / 2;
          if (
            router &&
            ...
            if (left.find('.back .icon ~ span').length) {
              const leftOffset = left[0].cbxNavbarLeftOffset;
              const rightOffset = left[0].cbxNavbarRightOffset;
              left[0].cbxNavbarLeftOffset = 0;
              left[0].cbxNavbarRightOffset = 0;
              left.find('.back .icon ~ span')[0].cbxNavbarLeftOffset = leftOffset;
              left.find('.back .icon ~ span')[0].cbxNavbarRightOffset =
      ...
      if (!noRight && (right.hasClass('sliding') || sliding)) {
        if (app.rtl) {
          right[0].cbxNavbarLeftOffset = -rightWidth * inverter;
          right[0].cbxNavbarRightOffset =
            ((navbarInnerWidth - right[0].offsetWidth) / 2) * inverter;
        } else {
          right[0].cbxNavbarLeftOffset = -(navbarInnerWidth - right[0].offsetWidth) / 2;
          right[0].cbxNavbarRightOffset = rightWidth;
        }
      }
      if (subnavbar.length && (subnavbar.hasClass('sliding') || sliding)) {
        subnavbar[0].cbxNavbarLeftOffset = app.rtl
          ? subnavbar[0].offsetWidth
          : -subnavbar[0].offsetWidth;
        subnavbar[0].cbxNavbarRightOffset = -subnavbar[0].cbxNavbarLeftOffset;
      }
    }
  ...
  getElByPage(page) {
    ...
    if (page.$navbarEl || page.$el) {
      ...
      $pageEl = $(page);
      if ($pageEl.length > 0) pageData = $pageEl[0].cbxPage;
    }
  ...
  getPageByEl(navbarEl) {
    ...
    $navbarEl
      ...
      .find('.page')
      .each((el) => {
        if (el && el.cbxPage && el.cbxPage.navbarEl && $navbarEl[0] === el.cbxPage.navbarEl) {
          pageEl = el;
        }
      });
    return pageEl;
  ...
  initNavbarOnScroll(pageEl, navbarEl, needHide, needCollapse, needTransparent) {
    ...
    if (needCollapse || (needHide && isLarge)) {
      navbarTitleLargeHeight = $navbarEl.css('--cbx-navbar-large-title-height');
    ...
    function calcScrollableDistance() {
      $pageEl.find('.page-content').each((pageContentEl) => {
        pageContentEl.cbxScrollableDistance =
    ...
    function handleLargeNavbarCollapse(pageContentEl) {
      ...
      const scrollableDistance = Math.min(
        ...
        pageContentEl.cbxScrollableDistance || navbarTitleLargeHeight,
      );
    ...
    $pageEl[0].cbxDetachNavbarScrollHandlers = function cbxDetachNavbarScrollHandlers() {
      app.off('resize', calcScrollableDistance);
      delete $pageEl[0].cbxDetachNavbarScrollHandlers;
...
export default {
  ...
  on: {
    ...
    pageBeforeRemove(page) {
      if (page.$el[0].cbxDetachNavbarScrollHandlers) {
        page.$el[0].cbxDetachNavbarScrollHandlers();
      }
    },
    pageBeforeIn(page) {
      const app = this;
      ...
      let $navbarsEl;
      const view = page.$el.parents('.view')[0].cbxView;
