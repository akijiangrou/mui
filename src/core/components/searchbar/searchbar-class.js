...
class Searchbar extends FrameworkClass {
  constructor(app, params = {}) {
    ...
    if ($el[0].cbxSearchbar) return $el[0].cbxSearchbar;

    $el[0].cbxSearchbar = sb;
    ...
    if ($el.parents('.page').length > 0) {
      ...
      if (!$pageEl.length) {
        ...
        if (
          $currentPageEl[0] &&
          $currentPageEl[0].cbxPage &&
          $currentPageEl[0].cbxPage.navbarEl === $navbarEl[0]
        ) {
  ...
  search(query, internal) {
    ...
    let vlQuery;
    if (isVirtualList) {
      sb.virtualList = $searchContainer[0].cbxVirtualList;
      if (query.trim() === '') {
  ...
  destroy() {
    ...
    sb.detachEvents();
    if (sb.$el[0]) {
      sb.$el[0].cbxSearchbar = null;
      delete sb.$el[0].cbxSearchbar;
    }
    deleteProps(sb);
  }
}
