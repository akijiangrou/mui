...
class Popup extends Modal {
  constructor(app, params) {
    ...
    if ($el && $el.length > 0 && $el[0].cbxModal) {
      return $el[0].cbxModal;
    }
      ...
      if ($target.closest(popup.el).length === 0) {
        if (
          ...
          popup.$el.nextAll('.popup.modal-in').each((popupEl) => {
            const popupInstance = popupEl.cbxModal;
          ...
          if ($pushEl.hasClass('popup-push')) {
            $pushEl.transition(0).forEach((el) => {
              el.style.setProperty(
                'transform',
                `translate3d(0, calc(-1 * ${pushProgress} * (var(--cbx-popup-push-offset) + 10px)) , 0px) scale(${scale})`,
                'important',
              );
            });
          } else {
    ...
    const updatePushOffset = () => {
      ...
      if (isPush && !wasPush) {
        ...
        popup.$htmlEl[0].style.setProperty('--cbx-popup-push-scale', pushViewScale(pushOffset));
      } else if (!isPush && wasPush) {
        popup.$htmlEl.removeClass('with-modal-popup-push');
        popup.$htmlEl[0].style.removeProperty('--cbx-popup-push-scale');
      }
    };
    ...
    const setPushOffset = () => {
      ...
      if (isPush) {
        pushOffset = parseInt($el.css('--cbx-popup-push-offset'), 10);
