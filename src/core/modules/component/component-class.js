...
class Component {
    ...
    const getRenderFuncion = (componentResult) =>
      new Promise((resolve, reject) => {
        if (typeof componentResult === 'function') {
          ...
          reject(
            new Error(
              'Xui: Component render function is not a "function" type. Didn\'t you forget to "return $render"?',
            ),
          );
        }
      });
  ...
  getComponentStore() {
    const { state, _gettersPlain, dispatch } = this.cbx.store;
    const $store = {
      state,
      dispatch,
    };
  ...
  getComponentContext(includeHooks) {
    const ctx = {
      $cbxroute: this.context.cbxroute,
      $cbxrouter: this.context.cbxrouter,
      $h,
      $,
      $id: this.id,
      $cbx: this.cbx,
      $cbxready: this.cbxready.bind(this),
      $theme: this.theme,
  ...
  destroy() {
    ...
    if (this.el && this.el.cbxComponent) {
      this.el.cbxComponent = null;
      delete this.el.cbxComponent;
    }
