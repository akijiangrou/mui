...
const createCustomComponent = ({ cbx, treeNode, vnode, data }) => {
  const component =
...
function getHooks(treeNode, data, cbx, initial, isRoot) {
  const hooks = {};
  const insert = [];
  const destroy = [];
  const update = [];
  ...
  if (isCustom) {
    insert.push((vnode) => {
      ...
      createCustomComponent({ cbx, treeNode, vnode, data });
    });
    destroy.push((vnode) => {
  ...
  if (!isCustom) {
    ...
    classNames.split(' ').forEach((className) => {
      if (!initial) {
        insert.push(...cbx.getVnodeHooks('insert', className));
      }
      destroy.push(...cbx.getVnodeHooks('destroy', className));
      update.push(...cbx.getVnodeHooks('update', className));
      postpatch.push(...cbx.getVnodeHooks('postpatch', className));
    });
  }
...
const getData = (treeNode, component, cbx, initial, isRoot) => {
  ...
  const hooks = getHooks(treeNode, data, cbx, initial, isRoot);
...
const getChildren = (treeNode, component, cbx, initial) => {
  ...
  for (let i = 0; i < nodes.length; i += 1) {
    ...
    const child = treeNodeToVNode(childNode, component, cbx, initial, false);
...
const getSlots = (treeNode, component, cbx, initial) => {
  ...
  if (slotNodes.length === 0) {
    return getChildren(treeNode, component, cbx, initial);
  }
  return slotNodes.map((subTreeNode) => treeNodeToVNode(subTreeNode, component, cbx, initial));
};
...
const treeNodeToVNode = (treeNode, component, cbx, initial, isRoot) => {
  if (!isTreeNode(treeNode)) {
    return String(treeNode);
  }
  if (treeNode.type === 'slot') {
    return getSlots(treeNode, component, cbx, initial);
  }
  const data = getData(treeNode, component, cbx, initial, isRoot);
  const children = isCustomComponent(treeNode.type)
    ? []
    : getChildren(treeNode, component, cbx, initial);

  return h(getTagName(treeNode), data, children);
};

export default function vdom(tree = {}, component, initial) {
  return treeNodeToVNode(tree, component, component.cbx, initial, true);
}
