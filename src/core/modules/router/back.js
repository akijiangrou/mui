...
function backward(router, el, backwardOptions) {
  ...
  if (dynamicNavbar) {
    ...
    if ($newNavbarEl.length === 0 && $newPage[0] && $newPage[0].cbxPage) {
      // Try from pageData
      $newNavbarEl = $newPage[0].cbxPage.$navbarEl;
    }
  ...
  if (masterDetailEnabled) {
    ...
    isDetail =
      ...
      router.history.indexOf(options.route.url) >
        router.history.indexOf(masterPageEl.cbxPage.route.url);

    if (
      !isDetail &&
      !isMaster &&
      masterPageEl &&
      masterPageEl.cbxPage &&
      options.route.route.masterRoute
    ) {
      isDetail = options.route.route.masterRoute.path === masterPageEl.cbxPage.route.route.path;
    }
  }
  if (isDetail && masterPageEl && masterPageEl.cbxPage) {
    isDetailRoot =
      router.history.indexOf(options.route.url) -
        router.history.indexOf(masterPageEl.cbxPage.route.url) ===
      1;
  ...
  const cbxComponent = $newPage[0].cbxComponent;

  function insertPage() {
    if (initialPreload) {
      if (!newPageInDom && cbxComponent) {
        cbxComponent.mount((componentEl) => {
    ...
    if (!newPageInDom) {
      ...
      options.route.route &&
      options.route.route.keepAlive &&
      !$newPage[0].cbxPageMounted
    ) {
      $newPage[0].cbxPageMounted = true;
      router.pageCallback(
        'mounted',
        $newPage,
        $newNavbarEl,
  ...
  if (options.animate && !(currentIsMaster && app.width >= router.params.masterDetailBreakpoint)) {
    ...
    if ($oldPage[0] && $oldPage[0].cbxPageTransition) {
      transition = $oldPage[0].cbxPageTransition;
      delete $oldPage[0].cbxPageTransition;
    }
...
function loadBack(router, backParams, backOptions, ignorePageChange) {
  ...
  if (content) {
    ...
    // Load from component (cbx/Vue/React/...)
    try {
...
function back(...args) {
  ...
  if (router.params.masterDetailBreakpoint > 0) {
    ...
    if ($previousMaster.length) {
      ...
      if (
        ...
        $previousMaster[0].cbxPage &&
        expectedPreviousPageRoute.route === $previousMaster[0].cbxPage.route.route
      ) {
  ...
  if (!navigateOptions.force && $previousPage.length && !skipMaster) {
    if (
      ...
      $previousPage[0].cbxPage &&
      router.history[router.history.length - 2] !== $previousPage[0].cbxPage.route.url
    ) {
      router.back(
    ...
    const previousPageRoute = $previousPage[0].cbxPage.route;
  ...
  if (options.force && router.params.stackPages) {
    router.$el.children('.page-previous.stacked').each((pageEl) => {
      if (pageEl.cbxPage && pageEl.cbxPage.route && pageEl.cbxPage.route.url === route.url) {
