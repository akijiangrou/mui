...
function forward(router, el, forwardOptions = {}) {
  ...
  if (options.reloadCurrent || options.reloadAll || reloadDetail) {
    ...
    if (reloadDetail) {
      ...
      if (masterPageEl.cbxPage && masterPageEl.cbxPage.navbarEl) {
        router.setNavbarPosition($(masterPageEl.cbxPage.navbarEl), 'previous');
      }
    }
    return router;
  }
  ...
  if (options.animate && !(isMaster && app.width >= router.params.masterDetailBreakpoint)) {
    ...
    if (transition) {
      $newPage[0].cbxPageTransition = transition;
    }
...
function navigate(navigateParams, navigateOptions = {}) {
  ...
  if (route.route && route.route.viewName) {
    ...
    if (!anotherView) {
      throw new Error(
        `Xui: There is no View with "${anotherViewName}" name that was specified in this route`,
      );
    }
