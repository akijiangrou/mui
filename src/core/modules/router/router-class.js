...
class Router extends XuiClass {
  ...
  animatableNavElements($newNavbarEl, $oldNavbarEl, toLarge, fromLarge, direction) {
    ...
    function animatableNavEl($el, $navbarInner) {
      ...
      return {
        $el,
        isIconLabel,
        leftOffset: $el[0].cbxNavbarLeftOffset,
        rightOffset: $el[0].cbxNavbarRightOffset,
        isSliding,
        isSubnavbar,
        ...
        if (navEl.isSliding) {
          if (navEl.isSubnavbar && newIsLarge) {
            ...
            $el[0].style.setProperty(
              'transform',
              `translate3d(${offset * (1 - progress)}px, calc(-1 * var(--cbx-navbar-large-collapse-progress) * var(--cbx-navbar-large-title-height)), 0)`,
              'important',
            );
          } else {
        ...
        if (navEl.isSliding) {
          if (navEl.isSubnavbar && oldIsLarge) {
            $el.transform(
              `translate3d(${
                offset * progress
              }px, calc(-1 * var(--cbx-navbar-large-collapse-progress) * var(--cbx-navbar-large-title-height)), 0)`,
            );
          } else {
  ...
  removePage(el) {
    const $el = $(el);
    const cbxPage = $el && $el[0] && $el[0].cbxPage;
    const router = this;
  ...
  removeEl(el) {
    ...
    $el.find('.tab').each((tabEl) => {
      $(tabEl)
        .children()
        .each((tabChild) => {
          if (tabChild.cbxComponent) {
            $(tabChild).trigger('tab:beforeremove');
            tabChild.cbxComponent.destroy();
          }
        });
    });
    if ($el[0].cbxComponent && $el[0].cbxComponent.destroy) {
      $el[0].cbxComponent.destroy();
    }
  ...
  generateUrl(parameters = {}) {
    ...
    if (!name && !path) {
      throw new Error('Xui: "name" or "path" parameter is required');
    }
    const router = this;
    ...
    if (!route) {
      if (name) {
        throw new Error(`Xui: route with name "${name}" not found`);
      } else {
        throw new Error(`Xui: route with path "${path}" not found`);
      }
    }
    const url = router.constructRouteUrl(route, { params, query });
    if (!url) {
      throw new Error(`Xui: can't construct URL for route with name "${name}"`);
    }
    return url;
  }
  ...
  constructRouteUrl(route, { params, query } = {}) {
    ...
    try {
      ...
      throw new Error(
        `Xui: error constructing route URL from passed params:\nRoute: ${path}\n${error.toString()}`,
      );
    }
  ...
  getPageData(pageEl, navbarEl, from, to, route = {}, pageFromEl) {
    ...
    const currentPage = $pageEl[0].cbxPage || {};
    let direction;
    let pageFrom;
  ...
  pageCallback(callback, pageEl, navbarEl, from, to, options = {}, pageFromEl) {
    ...
    const keepAlive =
      $pageEl[0].cbxPage &&
      $pageEl[0].cbxPage.route &&
      $pageEl[0].cbxPage.route.route &&
      $pageEl[0].cbxPage.route.route.keepAlive;
    ...
    let page = {};
    if (callback === 'beforeRemove' && $pageEl[0].cbxPage) {
      page = extend($pageEl[0].cbxPage, { from, to, position: from });
    } else {
    ...
    function attachEvents() {
      if ($pageEl[0].cbxRouteEventsAttached) return;
      $pageEl[0].cbxRouteEventsAttached = true;
      if (on && Object.keys(on).length > 0) {
        $pageEl[0].cbxRouteEventsOn = on;
      ...
      if (once && Object.keys(once).length > 0) {
        $pageEl[0].cbxRouteEventsOnce = once;
    ...
    function detachEvents() {
      if (!$pageEl[0].cbxRouteEventsAttached) return;
      if ($pageEl[0].cbxRouteEventsOn) {
        Object.keys($pageEl[0].cbxRouteEventsOn).forEach((eventName) => {
          $pageEl.off(eventNameToColonCase(eventName), $pageEl[0].cbxRouteEventsOn[eventName]);
        });
      }
      if ($pageEl[0].cbxRouteEventsOnce) {
        Object.keys($pageEl[0].cbxRouteEventsOnce).forEach((eventName) => {
          $pageEl.off(eventNameToColonCase(eventName), $pageEl[0].cbxRouteEventsOnce[eventName]);
        });
      }
      $pageEl[0].cbxRouteEventsAttached = null;
      $pageEl[0].cbxRouteEventsOn = null;
      $pageEl[0].cbxRouteEventsOnce = null;
      delete $pageEl[0].cbxRouteEventsAttached;
      delete $pageEl[0].cbxRouteEventsOn;
      delete $pageEl[0].cbxRouteEventsOnce;
    }
    ...
    if (callback === 'init') {
      ...
      attachEvents();
      if ($pageEl[0].cbxPageInitialized) {
        $pageEl.trigger('page:reinit', page);
        router.emit('pageReinit', page);
        return;
      }
      $pageEl[0].cbxPageInitialized = true;
    }
    if (
      restoreScrollTopOnBack &&
      callback === 'beforeOut' &&
    ...
    if (callback === 'beforeRemove' || callback === 'beforeUnmount') {
      detachEvents();
      if (!keepAlive) {
        if ($pageEl[0].cbxPage && $pageEl[0].cbxPage.navbarEl) {
          delete $pageEl[0].cbxPage.navbarEl.cbxPage;
        }
        $pageEl[0].cbxPage = null;
      }
    }
  }
  ...
  saveHistory() {
    ...
    if (
      ...
      window.localStorage[`cbxrouter-${router.view.id}-history`] = JSON.stringify(router.history);
    }
  }
  ...
  restoreHistory() {
    ...
    if (
      ...
      router.params.browserHistoryStoreHistory &&
      window.localStorage &&
      window.localStorage[`cbxrouter-${router.view.id}-history`]
    ) {
      router.history = JSON.parse(window.localStorage[`cbxrouter-${router.view.id}-history`]);
      router.view.history = router.history;
    }
  }
  ...
  getInitialUrl() {
    ...
    if (
      ...
      // eslint-disable-next-line
      console.warn(
        'Xui: wrong or not complete browserHistory configuration, trying to guess browserHistoryRoot',
      );
