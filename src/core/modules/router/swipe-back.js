...
function SwipeBack(r) {
  ...
  const transformOriginTitleLarge = app.rtl
    ? 'calc(100% - var(--cbx-navbar-large-title-padding-left) - var(--cbx-safe-area-left)) center'
    : 'calc(var(--cbx-navbar-large-title-padding-left) + var(--cbx-safe-area-left)) center';
  ...
  function animatableNavElements() {
    ...
    if (params.iosAnimateNavbarBackIcon) {
      ...
      if (activeNavBackIconText.length) {
        $previousNavElements.each((el) => {
          ...
          el.cbxNavbarLeftOffset += activeNavBackIconText.prev('.icon')[0].offsetWidth;
        });
      }
    }
    $currentNavElements.each((navEl) => {
        ...
        if (isLeft) {
          ...
          $navEl.find('.back span').each((subNavEl) => {
            els.push({
              ...
              transform: (progress) =>
                `translateX(calc(${progress} * (var(--cbx-navbarTitleLargeOffset) - var(--cbx-navbarLeftTextOffset)))) translateY(calc(${progress} * (var(--cbx-navbar-large-title-height) - var(--cbx-navbar-large-title-padding-vertical) / 2))) scale(${
                  1 + 1 * progress
                })`,
            });
          });
      ...
      if (isBg) {
        ...
        if (!fromLarge && !toLarge) {
          if (currentNavIsCollapsed) {
            ...
            el.transform = (progress) =>
              ...
              }%) translateY(calc(-1 * var(--cbx-navbar-large-title-height)))`;
          } else {
        ...
        if (!fromLarge && toLarge) {
          ...
          el.transform = (progress) =>
            `translateX(${100 * progress * inverter}%) translateY(calc(-1 * ${
              1 - progress
            } * var(--cbx-navbar-large-title-height)))`;
        }
        if (fromLarge && toLarge) {
          el.transform = (progress) => `translateX(${100 * progress * inverter}%)`;
        }
        if (fromLarge && !toLarge) {
          el.transform = (progress) =>
            `translateX(${
              100 * progress * inverter
            }%) translateY(calc(-${progress} * var(--cbx-navbar-large-title-height)))`;
        }
        return;
      }
      ...
      if (isSliding) {
        ...
        transformTarget.transform = (progress) => {
          let activeNavTranslate = progress * transformTarget.el.cbxNavbarRightOffset;
          if (device.pixelRatio === 1) activeNavTranslate = Math.round(activeNavTranslate);
          if (isSubnavbar && currentNavIsLarge) {
            return `translate3d(${activeNavTranslate}px, calc(-1 * var(--cbx-navbar-large-collapse-progress) * var(--cbx-navbar-large-title-height)), 0)`;
          }
    ...
    $previousNavElements.each((navEl) => {
          ...
          $navEl.find('.title-large-text').each((subNavEl) => {
            els.push({
              ...
              transform: (progress) =>
                `translateX(calc(${
                  1 - progress
                } * (var(--cbx-navbarLeftTextOffset) - var(--cbx-navbarTitleLargeOffset)))) translateY(calc(${
                  progress - 1
                } * var(--cbx-navbar-large-title-height) + ${
                  1 - progress
                } * var(--cbx-navbar-large-title-padding-vertical))) scale(${
                  0.5 + progress * 0.5
                })`,
            });
          });
      ...
      if (isSliding) {
        ...
        transformTarget.transform = (progress) => {
          let previousNavTranslate = transformTarget.el.cbxNavbarLeftOffset * (1 - progress);
          if (device.pixelRatio === 1) previousNavTranslate = Math.round(previousNavTranslate);
          if (isSubnavbar && previousNavIsLarge) {
            return `translate3d(${previousNavTranslate}px, calc(-1 * var(--cbx-navbar-large-collapse-progress) * var(--cbx-navbar-large-title-height)), 0)`;
          }
  ...
  function handleTouchMove(e) {
    ...
    if (isScrolling || e.cbxPreventSwipeBack || app.preventSwipeBack) {
      isTouched = false;
      return;
    }
    if (!isMoved) {
  ...
  function handleTouchEnd() {
    ...
    if (pageChanged) {
      ...
      router.pageCallback(
        ...
        'previous',
        'current',
        { route: $previousPageEl[0].cbxPage.route, swipeBack: true },
        $currentPageEl[0],
      );
    ...
    $currentPageEl.transitionEnd(() => {
      ...
      if (pageChanged) {
        ...
        router.pageCallback('afterIn', $previousPageEl, $previousNavbarEl, 'previous', 'current', {
          route: $previousPageEl[0].cbxPage.route,
          swipeBack: true,
        });
