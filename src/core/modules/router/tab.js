...
function tabLoad(tabRoute, loadOptions = {}) {
  ...
  if ($newTabEl && $newTabEl.parents('.page').length > 0 && options.route) {
    const tabParentPageData = $newTabEl.parents('.page')[0].cbxPage;
  ...
  if ($newTabEl[0].cbxRouterTabLoaded) {
  ...
  function loadTab(loadTabParams, loadTabOptions) {
    ...
    function resolve(contentEl) {
      ...
      if (typeof contentEl === 'string') {
        ...
        $newTabEl.html('');
        if (contentEl.cbxComponent) {
          contentEl.cbxComponent.mount((componentEl) => {
      ...
      $newTabEl[0].cbxRouterTabLoaded = true;
...
function tabRemove($oldTabEl, $newTabEl, tabRoute) {
  ...
  if ($oldTabEl[0]) {
    $oldTabEl[0].cbxRouterTabLoaded = false;
    delete $oldTabEl[0].cbxRouterTabLoaded;
  }
