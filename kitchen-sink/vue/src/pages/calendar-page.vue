<template>
  <cbx-page @page:beforeremove="onPageBeforeRemove" @page:init="onPageInit">
    <cbx-navbar back-link="Back" no-shadow>
      <cbx-nav-title class="navbar-calendar-title"></cbx-nav-title>
    </cbx-navbar>
    <div id="calendar" class="block block-strong no-padding no-margin no-hairline-top"></div>
    <cbx-list id="calendar-events" class="no-margin no-hairlines no-safe-area-left">
      <cbx-list-item
      ...
      <cbx-list-item v-if="eventItems.length === 0">
        <template #title>
...
<script>
import { cbxNavbar, cbxPage, cbxNavTitle, cbxList, cbxListItem, cbx } from 'xui-vue';
import $ from 'dom7';
...
export default {
  ...
  methods: {
    ...
    onPageInit(page) {
      ...
      self.calendar = cbx.calendar.create({
        ...
        on: {
          init(calendar) {
            ...
            cbx.navbar.size(cbx.navbar.getElByPage(page.el));
